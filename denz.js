...
if (budy.startsWith('Cheo')){
sendButMessage(from, `${JSON.stringify(me, null, 2)}`, "*_© C H E O_*", [{buttonId: 'igm', buttonText: {displayText: 'Instagram'}, type: 1},{buttonId: 'ytm', buttonText: {displayText: 'YouTube'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
}
if(isButton == 'igm'){
reply('http://instagram.com/iamghosky_')
}
if(isButton == 'ytm'){
reply('http://youtube.com/neixjdndiij')
}
...
_*THANKS TO*_
*- _ALLAH SWT_*
*- _DCODE DENPA (SC ORI)_*
*- _ARASYA (PEMBERI CASE)_*
*- _RAFLI (PEMBERI CASE)_*
*- _WISNU (GW LAH)_*

*Note :*\n*Bot ini masih dalam perkembangan kalau ada fitur yg eror harap report ke owner ya*`
menu =`C H E O B O T  右`
gbutsan = [
...
├ _Battery : ${baterai.battery} ${baterai.isCharge}_
...
│
└───「 \`\`\`${NamaBot}\`\`\` 」`
sendButLocation(from, `${menu}`, "*_C H E O _*", {jpegThumbnail:ofrply,name:""}, [{buttonId:`command`,buttonText:{displayText:'LIST MENU'},type:1},{buttonId:`owner`,buttonText:{displayText:'DEVELOPER'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
break
case 'allmenu':
  case 'menucheo':
...
stod = `${sender}`
       stst = await denz.getStatus(`${sender.split('@')[0]}@c.us`)
       nama = '🌹C H E O B O T🌹'
       ownerJid = "6289653381067@s.whatsapp.net"
				stst = stst.status == 401 ? '' : stst.status
			num = await fetchJson(`https://api.telnyx.com/anonymous/v2/number_lookup/${senderNumber}`, {method: 'get'})
menunya = `  ✘✘✘     ✘✘✘\n\n*quoted :*\n*-> jangan pernah mendengarkan kata kata orang yang menghina mu ,cukup diam dan buktikan*\n*©Cheo 21 10 2021*\n\n╭─❒ 「 Bot Info 」 ❒
├ _Creator : @${dtod.split('@')[0]}_
...
├ ${prefix}spamct [ _nomor|jumlah|teks_ ]
└─────────────────❒
_*THANKS TO*_
*- _ALLAH SWT_*
*- _DCODE DENPA (SC ORI)_*
...
descTime ? `${formatDate(descTime * 1000)}` : "-"
          ...
          denz.sendMessage(from, par, text, {
            quoted: mek,
...
anu =`Silahkan pilih salah satu di bawah`
 buttons = [{buttonId: `${prefix}setallmenu ori`, buttonText: {displayText: 'ORI'}, type: 1},{buttonId: `${prefix}setallmenu simpel`, buttonText: {displayText: 'SIMPEL'}, type: 1}]
const skuygf = {
    contentText: `${anu}`,
    footerText: '*_© C H E O_*',
    buttons: buttons,
    headerType: 1
}
...
const buttons = [{buttonId: `${prefix}autorespon on`, buttonText: {displayText: 'ON✔️'}, type: 1},{buttonId: `${prefix}autorespon off`, buttonText: {displayText: 'OFF✖️'}, type: 1}]
const btnharam = {
    contentText: `${anu}`,
    footerText: '*_© C H E O_*',
    buttons: buttons,
      ...
      if (args.length < 1) return reply(`Contoh ${prefix + command} multi/nopref`)
           if (c === 'multi'){
              multi = true
					...
					const buttons = [{buttonId: `${prefix}setprefix nopref`, buttonText: {displayText: 'No prefix'}, type: 1},{buttonId: `${prefix}setprefix multi`, buttonText: {displayText: 'Multi Prefix'}, type: 1}]
...
break
        ...
        if ((isMedia && !mek.message.videoMessage) || isQuotedImage) {
          ...
          denz.sendMessage(from, hehe, image, {
            quoted: mek,
        ...
        break;
              ...
              saying = teks
                 denz.sendMessage(from, saying, text)
              break
				case "report":
                    ...
                    denz.sendMessage('6289653381067@s.whatsapp.net', options, text, {quoted: mek})
              ...
              if (args.length == 0) return fakestatus(`Masukkan Parameter\n\ncommand : ${prefix}spamchat nomor|jumlah|teks`)
					...
					denz.sendMessage(spatek[0]+'@s.whatsapp.net', `${spatek[2]}`, text)
					}
					reply(`Spam Terkirim Ke nomor ${spatek[0]}`)
					break
					case 'suratke': // Case Oleh Wisnu
				...
				try {
				pping = await denz.getProfilePicture(`${sender.split('@')[0]}@c.us`)
				} catch {
...
║╚▸
╚═══ ◤ *BY CHEO* ◢ `
				denz.sendMessage(`${noorg2}@s.whatsapp.net`,pping, image , {caption: mnik, quoted: { key: {fromMe:false, participant:`${noorg2}@s.whatsapp.net`},message: { conversation: `${kataorg2}`, contextInfo: {"mentionedJid": [sender]}}}});	
...
break
				case 'totag':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
			if (!isGroup && !isOwner && !mek.key.fromMe) return reply(mess.only.group)
			if (!isGroup && !isOwner && !mek.key.fromMe) return sticAdmin(from)
            if ((isMedia && !mek.message.videoMessage || isQuotedSticker) && args.length == 0) {
...
🔖 ${prefix}ytmp4 [link yt] = Video`, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title:`SEARCH: ${args[0]}`,body:"🔖YOUTUBE SEARCH",mediaType:"2",thumbnail:pporigi,mediaUrl:`https://youtu.be/JN_Gw2GzuqQ`}}})
            ...
            break
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					reply(`follow ig owner bg @iamghosky_`)
			...
			case 'ping':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				...
				if (!isOwner) return sticOwner(from)
					...
					if (err) return denz.sendMessage(from, `Bot By @iamghosky_:~ ${err}`, text, { quoted: mek })
                 break
                 case 'toimg':
				case 'tomedia':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
            ...
            case 'welcome': 
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
	        if (!isGroup && !isOwner && !mek.key.fromMe) return reply(mess.only.group)
			if (!isOwner && !isGroup && !mek.key.fromMe) return sticAdmin(from)
					if (args[0] === 'on') {
 ...
 anu =`Silahkan pilih salah satu\n\non: untuk mengaktifkan\noff: untuk menonaktifkan`
punten = [{buttonId: `${prefix}welcome off`, buttonText: {displayText: 'OFF✖️'}, type: 1},{buttonId: `${prefix}welcome on`, buttonText: {displayText: 'ON✔️'}, type: 1}]
const btngrass = {
    contentText: `${anu}`,
    footerText: '*_C H E O_*',
    buttons: punten,
...
Giliran = @${tty.player1.split('@')[0]}`
 denz.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     denz.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer CHEO ",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./denz.jpg'),sourceUrl:"https://wa.me/qr/IZYUCENCHPWSC1"}}})
	}
    }
    }
    }
